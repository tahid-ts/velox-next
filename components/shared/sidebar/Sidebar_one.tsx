...
const Sidebar_one: React.FC<SidebarProps> = (props) => {
  const {
    sections,
    activeItemId,
    onItemClick,
          ...
          return (
            <div key={item.id} className="relative">
              <button
                onClick={() => !item.disabled && toggleSubmenu(item.id)}
                disabled={item.disabled}
                className="text-4xl font-cormorant my-2 cursor-pointer text-white hover:text-gray-200 transition-colors duration-200"
              >
                {content}
              </button>
            ...
            >
              {isFullScreen ? (
                <div className="flex flex-col items-center justify-start min-h-screen w-full px-6 py-12">
                  {/* Logo */}
                  <div className="flex w-[45%] gap-6 justify-between items-center mb-12">
                    {sidebarLogo ? (
                      sidebarLogo
                    ) : (
                      <>
        ...
        {toggleDeskNavBar ? (
          <div className={clsx("relative", Z.TOP, "pointer-events-none")}>
            <div className="flex justify-center pointer-events-auto">
              <div
                className={clsx(
                  "flex justify-between items-center w-full absolute lg:max-w-full   max-w-full  laptop:px-8 desktop-xl:mt-12.5 desktop-xl:px-11 bg-artboard desktop-lg:bg-transparent py-4 desktop-lg:py-0",
                  Z.TOP,
              ...
              >
                ...
                <div className={`flex gap-6.25 items-center ${Z.TOP} `}>
                  <div>
                    <LuUserRound className="text-2xl w-9 h-9 p-1.5" />
                  </div>
                  <button
